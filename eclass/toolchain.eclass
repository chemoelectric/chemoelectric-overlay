...
# Add Ada support.
# I used to provide an option to use a special bootstrapping compiler,
# but now leave reliable bootstrapping of gnat as an exercise for the reader.
# A given version of gnat may be picky about the compiler used.
# One possibility is to use my sys-devel/gnat-bootstrap, plus, via
# package.env, some settings for CC, CXX, GNATMAKE, and GNATBIND, to tell
# emerge to use the bootstrap compiler. You might also try using
# dev-lang/gnat-gcc.
[[ ${PN} == "gcc" ]] && IUSE+=" ada "
RESTRICT="!test? ( test )"
...
toolchain_src_test() {
	...
	SANDBOX_ON=0 LD_PRELOAD= emake P= -k check
}
...
toolchain_src_install() {
	...
	if is_ada ; then
	        for gnattool in gnat gnatbind gnatchop gnatclean gnatfind \
	                gnatkr gnatlink gnatls gnatmake gnatname gnatprep gnatxref ; do
	                local toollink
	                if is_crosscompile
	                then
