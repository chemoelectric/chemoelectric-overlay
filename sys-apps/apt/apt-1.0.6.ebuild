...
#
# FIXME: This ebuild does not work.
#
...
SRC_URI="mirror://debian/pool/main/a/${PN}/${PN}_${PV}${MY_PVR}.tar.xz"
...
src_prepare() {
	...
	# Fix broken symlinks
	gnuconfig_update
	sed -e 's#xml/docbook/stylesheet/nwalsh#sgml/docbook/xsl-ns-stylesheets#' \
		-i doc/manpage-style.xsl || die
	sed -e 's#/usr/share/xml/docbook/stylesheet/docbook-xsl/#/usr/share/sgml/docbook/xsl-ns-stylesheets/#' \
		-i doc/*.xsl || die
	make buildlib/config.guess buildlib/config.sub || die
	eautoreconf

	# FIXME: Skip the doc subdirectory.
	sed -e '/(MAKE) -C doc/d' -i Makefile || die

	# FIXME: Skip the test subdirectory.
	sed -e '/(MAKE) -C test/d' -i Makefile || die
}
