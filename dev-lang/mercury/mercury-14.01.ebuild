...
# Until the Portage tree contains an ebuild for this version, use the
# patchset from the previous version.
PREVIOUS_VERSION=13.05.2

DESCRIPTION="Mercury is a modern general-purpose logic/functional programming language"
HOMEPAGE="http://www.mercurylang.org/index.html"
SRC_URI="http://dl.mercurylang.org/release/${MY_P}.tar.gz
	mirror://gentoo/${PN}-${PREVIOUS_VERSION}-gentoo-patchset-${PATCHSET_VER}.tar.gz"
...
#	EPATCH_FORCE=yes
#	EPATCH_SUFFIX=patch
#	epatch "${WORKDIR}"/${PREVIOUS_VERSION}

	cd "${S}"
	for patch in "${WORKDIR}"/${PREVIOUS_VERSION}/* ; do
		epatch -Np1 "${patch}"
	done
...
#	EPATCH_FORCE=yes
#	EPATCH_SUFFIX=patch
#	epatch "${WORKDIR}"/${PREVIOUS_VERSION}-mmc
	for patch in "${WORKDIR}"/${PREVIOUS_VERSION}-mmc/* ; do
		epatch -Np1 "${patch}"
	done
